...
public class SimpleGeneticAlgorithm12
{
	...
	public SimpleGeneticAlgorithm12(int ps,int gs,int nb,double mr,double cr)
	{
		nbits = nb;
		popsize = ps;
		gensize = gs;
	...
	public OneMaxChrome OneMaxRunSGA(boolean report)
	{
		InitPop();
	...
	public OneMaxChrome OneMaxRunSGA(int nfc,boolean report)
	{
		InitPop();
		int i = 0;
	...
	private void Report(int i,boolean report)
	{
		...
		System.out.print(" 	");
		System.out.println(population.get(0).OneMaxGetFitness());
	}
	...
	{
		double av = 0.0;
		for(int i=0;i<population.size();++i) av += population.get(i).OneMaxGetFitness();
	...
	private void DoOnePointCrossOver(int p1id,int p2id)
	{
		...
		int xopos = CS2004_12.UI(1,nbits-1);
	...
	private void CrossOver()
	{
		...
		for(int i=0;i<popsize;++i)
		{
			if (CS2004_12.UR(0.0,1.0) < crate) xlist.add(i);
		}
		...
		while(xlist.size() > 1)
		{
			...
			while(a >= b)
			{
				a = CS2004_12.UI(0,xlist.size()-1);
				b = CS2004_12.UI(0,xlist.size()-1);
			}
