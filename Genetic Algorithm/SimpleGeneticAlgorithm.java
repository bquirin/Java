...
public class SimpleGeneticAlgorithm
{
	...
	public SimpleGeneticAlgorithm(int popS,int gRun,int numW,double mutateR,double crossR)
	{
		numBits = numW;
		popsize = popS;
		gensize = gRun;
	...
	//If 'report' is true then display the best fitness for each generation
	public ScalesChrome RunSGA(int numberfc,boolean report)
	{
		InitPop();
		int i = 0;
		while(ScalesChrome.GetFC() < numberfc)
		{
			CrossOver();
			Mutation();
			Survival();
	...
	private void InitPop()
	{
		...
		{
			population.add(new ScalesChrome(numBits));
		}
	}
	...
	private void DoOnePointCrossOver(int p1id,int p2id)
	{
		...
		int xopos = CS2004_11.UI(1,numBits-1);
...
//------------------------------------------------------------------------------------			
	...
	private void CrossOver()
	{
		...
		for(int i=0;i<popsize;++i)
		{
			if (CS2004_11.UR(0.0,1.0) < crate) xlist.add(i);
		}
		...
		while(xlist.size() > 1)
		{
			...
			while(a >= b)
			{
				a = CS2004_11.UI(0,xlist.size()-1);
				b = CS2004_11.UI(0,xlist.size()-1);
			}
